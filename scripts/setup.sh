#!/bin/bash
# This Script is used to install or uninstall facsimile in the system
# pass --uninstall to uninstall the facsimile
# pass --help to get the help information
# pass --purge to uninstall the facsimile and remove the configuration files
...
if [ "$1" == "$HELP" ]; then
  ...
  echo "Install or Uninstall the facsimile Application"
  echo ""
  echo "Options:"
  echo "  $UNINSTALL uninstall the facsimile"
  echo "  $PURGE     uninstall the facsimile completely"
...
APP_NAME="facsimile.AppImage";

# Define the List of Variables
ICON_LOC="$APP_HOME/facsimile.png"
SH_LOC="$APP_HOME/facsimile.sh"
APP_LOC="$APP_HOME/$APP_NAME"
LOG_FILE="$APP_HOME/log.txt"

# Conf for .desktop file & auto start
APPDESKTOP_FILE="$HOME/.local/share/applications/facsimile.desktop"
AUTOSTART_DIR="$HOME/.config/autostart"
AUTOSTART_ENTRY="$AUTOSTART_DIR/facsimile.desktop"
